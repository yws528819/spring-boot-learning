...
@CacheConfig(cacheNames = {"emp"}, cacheManager = "empRedisCacheManager")//抽取缓存的公共配置
@Service
public class EmployeeService {
     ...
     *          如：key为getEmp[2]：  @Cacheable(cacheNames = {"emp"}, key = "#root.methodName + '[' + #id + ']'")
     *
     ...
     *                  unless="#result == null"
     *                  unless = "#a0==2"：如果第一个参数的值是2，返回结果不缓存
     *
     *      sync：是否使用异步模式
     *           开启后，#unless() is not supported（不支持）
     *
     * 原理：
     ...
     *      注意：@{@link Cacheable}的key是不能用#result ，因为执行方法前就要拿到key
     ...
     *      默认代表缓存清除操作是在方法执行之后执行；如果出现异常缓存是不会清除
     * beforeInvocation = true；
     *      代表缓存清除操作是在方法执行之前执行；无论方法是否出现异常，缓存都清除
     * @param id
     */
    //@CacheEvict(value = "emp", key = "#id")
    //@CacheEvict(value = "emp", allEntzries = true)
